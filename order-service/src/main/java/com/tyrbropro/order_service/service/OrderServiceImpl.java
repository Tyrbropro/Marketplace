    ...
    public OrderResponseDto createOrder(OrderRequestDto orderRequestDto) {
        ...
        if (orderDto == null || !"CUSTOMER".equals(orderDto.role())) {
            this.kafkaProducer.sendTopic("user_not_customer_topic",
                    OrderTopicDto.newBuilder()
                            .setUserId(orderRequestDto.customerId())
                            .setMessage("User not found or is not a CUSTOMER")
                            .setType("ERROR")
                            .setTime(Instant.now())
                            .build());
    ...
    public void acceptOrder(AcceptOrderDto acceptOrderDTO) {
        ...
        if (executorDto == null || !"EXECUTOR".equals(executorDto.role())) {
            this.kafkaProducer.sendTopic("user_not_executor_topic",
                    OrderTopicDto.newBuilder()
                            .setUserId(acceptOrderDTO.orderId())
                            .setMessage("User not found or is not a EXECUTOR")
                            .setType("ERROR")
                            .setTime(Instant.now())
                            .build());
